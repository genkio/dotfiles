...
- `brew/Brewfile.dev` — dev tools (ghostty, docker, gh, claude-code, codex, etc.) for the explicit `--include-dev` flow; version managers (volta, pyenv, rust) are installed via official curl installers in `setup-dev.sh`
