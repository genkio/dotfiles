...
- `tmux`, `mpv` and `gnupg` have no Intel bottle either and brew compiles them. `tmux` is a cheap C build (all its deps are bottled); `gnupg` pulls `libgcrypt`, and `mpv` pulls jpeg-turbo, little-cms2, luajit and mujs, so they are tens of minutes rather than hours. MacPorts is the intended home for these on Intel - `ports-14_x86_64` publishes Sonoma archives for `gnupg2` and `mpv` - and `awscli` is already guarded off Intel because its nine aws-c-* dependencies would build the whole C SDK.
