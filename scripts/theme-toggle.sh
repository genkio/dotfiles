...
# tmux, kitty's config, and the live terminal via OSC. Bound to tmux
# prefix+t: one manual flip, local or SSH, so macOS-appearance watching is moot.
# Revert to macOS appearance: rm the override + kitty-theme-active.conf under
...
# kitty: rewrite its active colors file; SIGUSR1 reload picks it up.
# Non-fatal like the tmux step so a cosmetic seed failure can't abort the toggle.
"$dotfiles/scripts/apply-kitty-theme.sh" || true
