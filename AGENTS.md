...
cd ~/dotfiles && stow kitty && bash scripts/apply-kitty-theme.sh
...
| `kitty` | `~/.config/kitty/` | Terminal emulator (Flexoki Light / TokyoNight Storm). Run `scripts/apply-kitty-theme.sh` after stow to seed the active theme; light/dark is driven by `theme-toggle.sh` (tmux `prefix + t`), which rewrites the active theme and repaints the running terminal via OSC |
...
- kitty config sets `clipboard_control` to allow OSC52 read+write
