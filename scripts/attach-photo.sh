...
cache=${ATTACH_PHOTO_DIR:-$HOME/.cache/attach-photo}
...
[ -n "$target_pane" ] || { attach_notify attach-photo "no target pane"; exit 0; }
for bin in fzf viu sqlite3; do
  command -v "$bin" >/dev/null 2>&1 || { attach_notify attach-photo "$bin not found"; exit 0; }
done
[ -d "$lib" ] || { attach_notify attach-photo "no library at $lib"; exit 0; }
sqlite3 "$db" 'select 1 from ZASSET limit 1' >/dev/null 2>&1 ||
  { attach_notify attach-photo "cannot read the library db (Full Disk Access?)"; exit 0; }
...
while IFS="$tab" read -r _ uuid orig name _; do
  [ -n "${uuid:-}" ] || continue
  [ "$key" != ctrl-o ] || attach_notify attach-photo "asking Photos for $name..."
  err=$(mktemp)
  ...
  attach_send "$target_pane" "$file"
done <<< "$picks"

[ -z "$notes" ] || attach_notify attach-photo "$notes"
