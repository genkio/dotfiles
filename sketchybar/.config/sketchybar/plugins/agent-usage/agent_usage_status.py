...
CODEX_EXPIRY_BUFFER_MS = 5 * 60 * 1000

# The pi coding agent bills the same ChatGPT pool through its openai-codex
# provider, so its credential store keeps the Codex segment alive on machines
# that never install the Codex CLI.
...
def load_pi_codex_auth() -> dict[str, Any] | None:
    # Without the CLI there is no way to refresh pi's token, and pi owns that
    # file, so a readable-but-unrefreshable store is worse than the fallbacks.
    binary = find_pi_binary()
    if not binary:
        return None
    payload = read_json_file(PI_AUTH_PATH)
    if not isinstance(payload, dict):
        return None
    entry = payload.get(PI_CODEX_PROVIDER)
    ...
    return {
        "source": "pi",
        "binary": binary,
        "payload": {"tokens": tokens, "expires_at_ms": entry.get("expires")},
    }


def load_codex_auth() -> dict[str, Any] | None:
    # pi first: it refreshes on every run, while a ~/.codex/auth.json left
    # behind by an uninstalled CLI only goes staler.
    pi_state = load_pi_codex_auth()
    if pi_state:
        return pi_state
...
def codex_needs_refresh(payload: dict[str, Any]) -> bool:
    ...
    if isinstance(expires_at_ms, (int, float)):
        return int(time.time() * 1000) >= int(expires_at_ms) - CODEX_EXPIRY_BUFFER_MS
...
def refresh_pi_codex_token(state: dict[str, Any]) -> str | None:
    try:
        result = subprocess.run(
            [
                state["binary"],
    ...
    # pi rewrites its store on refresh; re-read it for the new expiry.
    refreshed = load_pi_codex_auth()
    if refreshed:
        state["payload"] = refreshed["payload"]
    state["payload"]["tokens"]["access_token"] = access_token
    return access_token


def refresh_codex_access_token(state: dict[str, Any]) -> str | None:
    if state["source"] == "pi":
        return refresh_pi_codex_token(state)
